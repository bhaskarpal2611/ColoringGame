...
namespace DrawingGame
{
    public class PaintService : MonoBehaviour
    {
        ...
        // Fired when a touch actually lands on the paintable canvas (BG_FRAME) / lifts off it —
        // NOT the same as a raw touch-began, which fires for any touch anywhere on screen.
        public event Action OnPaintTouchBegin;
        public event Action OnPaintTouchEnd;
        ...
        private void Awake()
        {
            ...
            _paintController.OnStrokeBegin += () => OnPaintTouchBegin?.Invoke();
            _paintController.OnStrokeEnd += () => OnPaintTouchEnd?.Invoke();
        ...
        public bool CanUndo => _paintController.CanUndo;
        public void Undo() => _paintController.Undo();
